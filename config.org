  ...
  (defun my/load-modus-vivendi-theme () ""
         (progn (load-theme 'modus-vivendi t)
  	      (custom-theme-set-faces 'modus-vivendi
  				      ...
  				      '(eglot-highlight-symbol-face ((t (:background "#552f5f")))))
...
* Defaults
** spelling
I don't know the differences between the spellcheck programs, but I already have the english dictionary for hunspell installed.
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  ...
  (add-hook 'html-ts-mode-hook (lambda () (whitespace-mode 1)))
...
* eglot
"C-h ." to display documentation at point.
#+begin_src emacs-lisp
  (use-package eglot
    ...
    :config (add-to-list 'eglot-server-programs
  		       `(vue-ts-mode . ("rass" "vue"))))
#+end_src

* lsp-mode =disabled=
Replacing lsp-mode with eglot, so disabling for now (could be useful to keep around).
** main
#+begin_src emacs-lisp
  (use-package lsp-mode
    ...
    :custom (lsp-enable-indentation nil)
  	  ...
  	  (lsp-typescript-format-enable nil) ;; doesn't seem to use the right indentation
  	  (lsp-apply-edits-after-file-operations nil)) ;; keeps adding emacs backup files to tsconfig.json and messes with imports
#+end_src
...
#+begin_src emacs-lisp
  (use-package vue-ts-mode
    :ensure (:host github :repo "theschmocker/vue-ts-mode")
    :mode ("\\.vue\\'")
    :hook (vue-ts-mode . (lambda () (progn
  				    (setq-local indent-tabs-mode nil)
  				    (setq-local vue-ts-mode-indent-offset 2)))))
#+end_src
...
it to be that much faster so far (maybe I just need bigger projects).
...
* flycheck
Flycheck can now run its own lsp for linting.
For now I'm getting it to use a globally installed =oxlint= by default for vue/typescript projects.
Might need to change it (to eslint or a local version of the lint program) on a per project basis, which would mean adding something like this in .dir-locals.el:
#+begin_src emacs-lisp
  ;; ((nil . ((eval . (make-local-variable 'flycheck-lsp-servers))
  ;; 	 (eval . (setf (alist-get 'vue-ts-mode flycheck-lsp-servers)
  ;; 		       '(".../[project]/node_modules/.bin/oxlint" "--lsp"))))))
#+end_src

Getting flycheck to work with eglot and enabling annotate mode.
I was having an issue with the eldoc buffer opening when it shouldn't (on a timer rather than only when specifically invoked), so I've overridden some of the
flycheck functions to fix/work around it for now. More explanation in comments, but I'd like to look more into what flycheck is doing with =flycheck--eldoc-supress-doc-window=.
#+begin_src emacs-lisp
  (setq my/default-nodejs-lint-program '("oxlint" "--lsp"))

  (use-package flycheck
    :ensure t
    :init
    (global-flycheck-eglot-mode)
    (global-flycheck-lsp-mode)
    ...
    (add-to-list 'flycheck-lsp-servers
  	       (cons 'vue-ts-mode my/default-nodejs-lint-program))
    (setf (alist-get 'typescript-ts-mode flycheck-lsp-servers)
          my/default-nodejs-lint-program)
    ;; Enabling =global-flycheck-lsp-mode= causes the eldoc buffer to display when activated by a timer
    ;; e.g. you have a cursor over a line with an error and the eldoc buffer will suddenly display,
    ;; even though it should only do that if you call it interactively (i.e. not through a timer)
    ;; It seems to me like it's something to do with the =flycheck-display-errors-via-eldoc= function.
    ;; It always calls =eldoc-print-current-symbol-info= with interactive set to "t", which seems to be what
    ;; causes it so I'm just passing on whether it was called interactively from =flycheck-display-error-at-point=.
    ;; It looks like flycheck has some custom eldoc handling via =flycheck--eldoc-suppress-doc-window=,
    ;; so I'll need to look more into what it's doing but this seems to work for now.
  ...
  If there are no errors, clears the error messages at point."
  	...
  	(let ((errors (flycheck-overlay-errors-at (point))))
            (if errors
                (flycheck-display-errors errors (called-interactively-p 'any))
              (flycheck-clear-displayed-errors))))))

    (defun flycheck-display-errors (errors &optional interactive)
      "Display ERRORS using `flycheck-display-errors-function'."
      (when flycheck-display-errors-function
        (funcall flycheck-display-errors-function errors interactive)))

    (defun flycheck-display-errors-via-eldoc (_errors &optional interactive)
...
#+begin_src emacs-lisp
  (use-package corfu
    ...
    (add-hook 'multiple-cursors-enabled-hook (lambda () (global-corfu-mode -1)))
    (add-hook 'multiple-cursors-disabled-hook (lambda () (global-corfu-mode 1))))
#+end_src
